...
import { DataClientsByOrdersType } from "@/types/ClientsByOrdersType"
import { Flex } from "@chakra-ui/react"
import { useEffect, useState } from "react"

export const ClientMultipleSelector = ({
    data,
    onChange
}: {
    ...
    const handleTest = (fullname1: string, fullname2: string) => {
        const targets = data.filter(e => e.fullname === fullname1 || e.fullname === fullname2);

        // Obtener todos los nombres de usuarios únicos
        const allUserNames = Array.from(new Set(targets.map(user => user.fullname)));
        ...
        targets.forEach(user => {
            user.orders.forEach(order => {
                ...
                if (dailyMap[dateKey] && dailyMap[dateKey][user.fullname] !== undefined) {
                    dailyMap[dateKey][user.fullname]++;
                }
            });
        });
    ...
    return (
        <Flex gap={4}>
            <SelectField
                ...
                placeholder={fullname1 != '' ? fullname1 : "Elija un cliente"}
                onChange={(e) => {
            ...
            <SelectField
                ...
                placeholder={fullname2 != '' ? fullname2 : "Elija un cliente"}
                onChange={(e) => {
